...
def my_job(*args, **kwargs):
    ...
    subprocess.run(['curl','http://localhost:9000/healthcheck'],stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
    subprocess.run(['curl','http://localhost:9000/push_to_object_store'],stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
    subprocess.run(['mv','.*csv','./interface/'],stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
